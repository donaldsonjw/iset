...
(module iset
   (export (make-bit-vector size . o)
           (integer->bit-vector n)
           (bit-vector-copy vec)
           (bit-vector-grow! vec i)
           bit-vector?
           (bit-vector-ref vec i)
           (bit-vector-set! vec i x)
           (bit-vector-set vec i x)
           (bit-vector-length vec)
           (bit-vector-count vec)
           (bit-vector-and! a . args)
           (bit-vector-and a . args)
           (bit-vector-andc2! a . args)
           (bit-vector-andc2 a . args)
           (bit-vector-ior! a . args)
           (bit-vector-xor! a . args)
           (bit-vector-eqv! a . args)
           (bit-vector-eqv a . args)
           (bit-vector-nand! a . args)
           (bit-vector-nand a . args)
           (bit-vector-nor! a . args)
           (bit-vector-nor a . args)
           (bit-vector-shift-in-place! vec n)
           (bit-vector-shift! vec n)
           (bit-vector-shift vec n)
           (range->bit-vector start end)
           (bit-vector-empty? vec)
           (bit-vector-full? vec to)
           (make-iset . opt)
           (list->iset! ls iset)
           (list->iset ls . opt)
           (iset-copy iset)
           ...
           (iset= . args)
           (iset-empty? iset)
           (iset-contains? iset n)
           (iset-adjoin1! iset n)
           (iset-adjoin! iset . ls)
           (iset-delete! iset . args)
           (iset-delete iset . args)
           (iset-unfold f p g seed . opt)
           (iset-unfold! f p g seed base-is)
           (iset-for-each proc iset)
           (iset-map proc iset)
           (iset-filter! proc iset base-is)
           (iset-filter proc iset . opt)
           (iset->list iset)
           (iset-any proc iset)
           (iset-every proc iset)
           (iset-size iset)
           (iset-cursor iset)
           (iset-ref iset cur)
           ...
           (iset-difference a . args)
           (iset-diff+intersection! . args)
           (iset-diff+intersection . args)
           (iset-xor . args)
           (iset-xor! . args)
           (iset-write-code is . opt)
           (iset-dump iset))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Bigloo porting utils

(cond-expand
   (bigloo-jvm
    (define (uint8->fixnum v)
       (let ((res ((@ uint8->fixnum __r4_numbers_6_5_fixnum) v)))
          (bit-and #xFF res)))))
...
(define fxxor bit-xoru8)
(define fxand bit-andu8)
(define fxior bit-oru8)
(define fxnot bit-notu8)

(define (arithmetic-shift v::long by::long)
   (if (positive? by)
       (bit-lsh v by)
       (bit-rsh v (- 0 by))))
...
(define (subu8vector vec start end)
   (let ((new-vec (make-u8vector (- end start))))
      (u8vector-copy! new-vec 0 vec start end)
      new-vec))

;; based on chicken schemes integer-length but
;; limited to uint8 values
(define (uint8-length v::uint8)
   (let ((v::long (uint8->fixnum v))
         (y::long 0)
         (n::long 0))
      (bind-exit (return)
         (set! y (bit-rsh v 4))
...
(define (u8vector-fill! vec fill . opt)
  (let-optionals* opt ((start 0) (end (u8vector-length vec)))
    (do ((i start (+ i 1)))
        ((= i end))
        (u8vector-set! vec i (fixnum->uint8 fill)))))
...
(define-syntax u8vector-map!
  (syntax-rules ()
    ((u8vector-map! proc pad a rest)
     (cond
      ((not (u8vector? a))
       (iset-error "u8vector-map!: not a u8vector" a))
      (else
       (let lp ((a a)
         ...
         (cond
          ((null? ls)
           a)
          ((not (u8vector? (car ls)))
           (iset-error "u8vector-map!: not a u8vector" (car ls)))
          (else
           (let* ((b (car ls))
...
(define (make-bit-vector size . o)
  (let* ((fill (if (and (pair? o) (car o)) #b11111111 0))
         ...
         (res (make-u8vector len (fixnum->uint8 fill))))
    (if (zero? fill)
      res
      (let ((off (remainder size 8)))
        (if (not (zero? off))
            (u8vector-set! res (- len 1) (fixnum->uint8 (- (arithmetic-shift 1 off) 1))))
        res))))
...
(define (bit-vector-ref vec i)
  (let ((byte (quotient i 8))
    ...
    (and (< byte (u8vector-length vec))
         (not (zero? (bitwise-and (uint8->fixnum (u8vector-ref vec byte))
    ...
    (cond
      ((< byte len)
       (u8vector-set! vec byte
          (fixnum->uint8 (if x
                             (bitwise-ior (uint8->fixnum (u8vector-ref vec byte))
                                (arithmetic-shift 1 off))
                             (bitwise-and (uint8->fixnum (u8vector-ref vec byte))
...
(define (bit-vector-count vec) ; # of 1's
  (let lp ((i (- (u8vector-length vec) 1))
    ...
    (if (< i 0)
      acc
      (lp (- i 1) (+ acc (bit-count (uint8->fixnum (u8vector-ref vec i))))))))

(define (u8vector-not! v lo hi)
  (do ((i lo (+ i 1)))
      ((= i hi))
      (u8vector-set! v i (fixnum->uint8 (byte-not (uint8->fixnum (u8vector-ref v i)))))))
...
(define (bit-vector-shift-in-place! vec n)
  (if (not (zero? n))
    (let ((len (u8vector-length vec)))
      (cond
       ((= len 1)
        (u8vector-set! vec 0 (fixnum->uint8 (bitwise-and
                                               #b11111111
                                               (arithmetic-shift (uint8->fixnum (u8vector-ref vec 0)) n)))))
       ((positive? n)
          ...
          (cond
            ...
            (else
             (let* ((lo-mask (- (arithmetic-shift 1 (- 8 off)) 1))
                ...
                (u8vector-set! vec (- len 1) (fixnum->uint8 (get-lo (uint8->fixnum (u8vector-ref vec start)))))
               (let lp ((from (- start 1)) (to (- len 1)))
                 (if (negative? from)
                   (when (>= to 0)
                     (u8vector-fill! vec 0 0 to))
                   (let* ((from-val (uint8->fixnum (u8vector-ref vec from)))
                          (lo (get-lo from-val))
                          (hi (get-hi from-val)))
                     (when (positive? to)
                        (u8vector-set! vec (- to 1) (fixnum->uint8 lo)))
                     (u8vector-set! vec to (fixnum->uint8 (bitwise-ior hi (uint8->fixnum (u8vector-ref vec to)))))
                     (lp (- from 1) (- to 1))))))))))
       (else
        (let* ((byte (quotient (- n) 8))
               (off (remainder (- n) 8))
               (s-byte (+ byte 1))
               (s-off (- 8 off))
               (save (uint8->fixnum (u8vector-ref vec byte))))
          ...
          (u8vector-set!
           vec
           0
           (fixnum->uint8 (bitwise-ior (uint8->fixnum (u8vector-ref vec 0))
                             (bitwise-and
...
(define (bit-vector-empty? vec)
  (let lp ((i (- (u8vector-length vec) 1)))
    (or (< i 0)
        (and (zero? (uint8->fixnum (u8vector-ref vec i)))
             (lp (- i 1))))))
	  ...
	  (or (zero? off)
	      (and (< i len)
		   (let ((mask (- (arithmetic-shift 1 off) 1)))
                      (= mask (bitwise-and mask (uint8->fixnum (u8vector-ref vec i)))))))
	  (and (< i len)
...
;; This is much uglier than the original call/cc based implementation
;; but is much better for Bigloo given its inefficient call/cc.  
(define (iset-cursor iset)
   (let ((curr-node #f)
         (node-stack (list iset))
         (start 0)
         (end 0)
         (bits #f)
         (limit 0)
      ...
      (lambda (command)
         (define (next)
             ;; do we need to find the next iset node 
            (when (and (not curr-node)
                       (pair? node-stack))
               (set! curr-node (let lp ((is (peek node-stack)))
                                  (and-let* ((left (iset-left is)))
                                     (push! node-stack left)
                                     (lp left))
                                  (let ((res (pop! node-stack)))
                                     (and-let* ((right (iset-right is)))
                                        (push! node-stack right))
                                     res)))
               (set! start (iset-start curr-node))
               (set! end (iset-end curr-node))
               (set! bits (iset-bits curr-node))
               (set! limit (if bits (+ 1 (- end start)) (+ end 1)))
               (set! i (if bits 0 start)))
            ;; get next integer in the set
            ;; if a node is available
            (if curr-node
                (if (< i limit)
                 (begin
                    ;; is the value stored as a bit-vector
                    (if bits
                        (if (bit-vector-ref bits i)
                            (begin
                               (set! res (+ i start))
                               (set! i (+ i 1)))
                            ;; we do not have i stored in the
                            ;; bit-vector, increment and try again
                            (begin
                               (set! i (+ i 1))
                               (next)))
                        ;; or are we looking at a range
                        (begin
                           (set! res i)
                           (set! i (+ i 1)))))
                 
                 ;; we have exhausted the values in this node
                 ;; on to the next 
                 (begin
                    (set! curr-node #f)
                    (set! i 0)
